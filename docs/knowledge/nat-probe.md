...
- C->S1:P1 `[NAT-1S]{sessionId}{S2-HTTP}`，服务端看到客户端的公网端点并记为 C1:P1；同时服务端了解到客户端选择使用的备用服务器 S2
    - S1->S2 (HTTP) S1 请求服务器 S2 向客户端公网端点 C1:P1 发送 NAT 探测包，并在本次请求中获得了本轮可供使用的两个 UDP 端点 P1 和 P2，在请求中返回给 S1 的同时，接下来也会将此信息通过 UDP 发给 C
...
- 客户端 C 一直等待收包直到超时：
    - 收到任意一个包，验证 `C1:P1 == C2:P1`，相等说明映射为「端点无关」，否则进行第五轮测试

---

当前四轮测试未得出结论时补测第 3.1 和第 3.2 轮。
